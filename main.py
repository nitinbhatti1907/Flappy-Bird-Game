...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():
            #if user click on close button then close the game.
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()

            #if user click on space key or up arrow key then start game for them.
...
def mainGame():
    ...
    #Create 2 Pipe for bliting on the screen.
    ...
    playerFlapped = False #it is only true when brid is flapping
    ...
    while True:
        ...
        #add the new pipe when the first is about to cross leftmost part of the screen.
        ...
        #lets blit our sprites(image) now.
        SCREEN.blit(GAME_SPRITES['background'], (0,0))
...
def getRandomPipe():
    """
    Generate position of two pipes for biliting on screen
    """
    pipeHeight = GAME_SPRITES['pipe'][0].get_height()
...
if __name__ == '__main__':
    ...
    pygame.display.set_caption('Flappy Bird by NitinBhatti')
